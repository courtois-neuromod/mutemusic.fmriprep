...
#SBATCH --job-name=fmriprep_study-cneuromod.mutemusic_sub-06_ses-001.job
#SBATCH --output=/localscratch/bpinsard-beluga4/cneuromod.mutemusic.fmriprep/code/fmriprep_study-cneuromod.mutemusic_sub-06_ses-001.out
#SBATCH --error=/localscratch/bpinsard-beluga4/cneuromod.mutemusic.fmriprep/code/fmriprep_study-cneuromod.mutemusic_sub-06_ses-001.err
...
if [ -d sourcedata/freesurfer ] ; then
    flock --verbose /lustre03/project/rrg-pbellec/ria-beluga/alias/cneuromod.mutemusic.fmriprep/.datalad_lock datalad push -J 4 -d sourcedata/freesurfer --to origin
fi 
if [ -e $LOCAL_DATASET/workdir/fmriprep_wf/resource_monitor.json ] ; then cp $LOCAL_DATASET/workdir/fmriprep_wf/resource_monitor.json /scratch/bpinsard/fmriprep_study-cneuromod.mutemusic_sub-06_ses-001_resource_monitor.json ; fi 
if [ $fmriprep_exitcode -ne 0 ] ; then cp -R $LOCAL_DATASET /scratch/bpinsard/fmriprep_study-cneuromod.mutemusic_sub-06_ses-001 ; fi 
